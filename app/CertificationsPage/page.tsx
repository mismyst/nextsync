    ...
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10">
        ...
        <section className="py-14 px-8 md:px-16">
          <h2 className="text-5xl font-extrabold text-white mb-6 text-center">Certifications</h2>
          <p className="text-xl text-cyan-300 mb-12 text-center max-w-3xl mx-auto">Gain a competitive edge with industry-leading certifications in tech and business. Explore and unlock your next big opportunity!</p>
          ...
          <div className="flex justify-center mb-12">
            <div className="relative w-full max-w-lg">
              <input
                ...
                className="w-full py-4 px-6 text-xl text-black bg-white/90 backdrop-blur-sm rounded-xl border-2 border-cyan-500 focus:outline-none focus:ring-4 focus:ring-cyan-400/50 transition-all duration-300 shadow-lg"
                value={searchQuery}
                onChange={(e) => setSearchQuery(e.target.value)}
              />
              <div className="absolute right-4 top-1/2 transform -translate-y-1/2 text-cyan-600 opacity-60 group-hover:opacity-100 transition duration-200">
          ...
          <div className="flex justify-center items-center gap-8 mb-12">
            <button
              ...
              className="px-8 py-4 bg-cyan-600 text-white text-lg font-semibold rounded-full shadow-xl hover:bg-cyan-700 transition duration-300 ease-in-out transform hover:scale-110 disabled:bg-cyan-900/40 disabled:text-white/50"
            ...
            {paginatedCertifications.map((cert, index) => (
              ...
              >
                ...
                <div className="p-8 relative z-10 flex flex-col justify-between h-full">
                  ...
                  <div>
                    <p className="inline-block px-5 py-2 bg-white text-cyan-800 text-sm font-bold rounded-full mb-5">{cert.level}</p>
                    <h3 className="text-2xl font-extrabold text-white mb-4">{cert.company}</h3>
                    <h4 className="text-xl text-cyan-100 mb-4">{cert.title}</h4>
                    <p className="text-sm text-cyan-200">{cert.type} · {cert.duration} · {cert.salary}</p>
                  </div>
                </div>
              </div>
            ))}
